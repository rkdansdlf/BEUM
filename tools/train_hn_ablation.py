#!/usr/bin/env python
"""
Track 5: Train Hard-Negative Ablation Candidate Model (50 Epochs, overlap_mask=False, mask_ratio=4).
Trains on canonical_2class_hn5 or canonical_2class_hn10, evaluates with canonical contract,
packages best weights to models/candidates/, and creates the formal model card JSON.
"""
...
def main():
    ...
    parser.add_argument("--hn-ratio", type=int, choices=[0, 5, 10, 20], default=5,
                        help="Hard Negative ratio (0, 5, 10, 20)")
    parser.add_argument("--base-model", type=str, default="models/best-seg-canonical.pt",
                        help="Base model checkpoint to start training from")
    ...
    data_yaml = Path(f"dataset/canonical_2class_hn{args.hn_ratio}/data.yaml")
    ...
    run_name = f"candidate_2class_hn{args.hn_ratio}_omfalse_mr{args.mask_ratio}_e{args.epochs}_s{args.seed}_{args.git_sha}"
    print("=" * 80)
    print(f"TRACK 5: TRAINING HARD NEGATIVE CANDIDATE (HN {args.hn_ratio}%): {run_name}")
    ...
    candidate_filename = f"best-seg-2class-hn{args.hn_ratio}-omfalse-mr{args.mask_ratio}-e{args.epochs}-s{args.seed}-{args.git_sha}.pt"
    ...
    card = {
        ...
        "hn_ratio_percent": args.hn_ratio,
    ...
    print("\n" + "=" * 80)
    print(f"TRACK 5: HN {args.hn_ratio}% CANDIDATE MODEL PACKAGED SUCCESSFULLY!")
