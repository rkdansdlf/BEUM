#!/usr/bin/env python
"""
P5: Train Final Candidate Model (50 Epochs, overlap_mask=False, mask_ratio=4).
Saves intermediate checkpoints, packages the final best weights to models/candidates/,
and creates the formal model card JSON.
"""
...
def main():
    ...
    run_name = f"candidate_omfalse_mr{args.mask_ratio}_e{args.epochs}_s{args.seed}_{args.git_sha}"
    print("=" * 80)
    print(f"P5: TRAINING FINAL CANDIDATE: {run_name}")
    ...
    print(f"\nEvaluating final candidate {best_pt} with Canonical False GT...")
    ...
    # Copy to models/candidates
    candidate_filename = f"best-seg-3class-omfalse-mr{args.mask_ratio}-e{args.epochs}-s{args.seed}-{args.git_sha}.pt"
    ...
    print("\n" + "=" * 80)
    print("FINAL CANDIDATE MODEL PACKAGED SUCCESSFULLY!")
